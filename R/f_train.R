...
f_prediction_intervall = function( df, pred_col, obs_col, intervall = 0.95, n_neighbours = 500,
  ...
  df = plyr::arrange(df, df[,pred_col] )
  ...
  if (steps == T){
    ...
    df$steps = cut2(df[,pred_col], m = n_neighbours)
  ...
  for (i in iterate) {
    ...
    if(bootstrap == T){
      ...
      returns = boot(data = sample, statistic = def_bootstrap
