...
This is basically a technique that can be applied if we have a binary response variable and any kind of predictor variable. We perform a reasonable binning on the response variable. Then we decide which form of the binary response we count as positive and which as negative and then calculate the percentage positive cases in each bin of all positive cases 
for example 20 out of 100 positive cases equals 20 %. Then we calculate the percentage of negative cases in each bin of all negative cases, for example 5 out of 50 cases equal 10%. Then we calculate the WOE by dividing the bin percentages of positive cases by the bin percentage of negative cases, and take the logarithm. log(20/10).
...
By doing another sequence of calculations similar to the WOE calculation we can calculate the IV. Classically this serves as variable ranking and allows us to perform feature selection, which is less compuationally demanding as other methods.
...
`scorecard::woebin()` returns a list with one element for each variable. There also is a plotting function that we can use to make meaninful plots and check the binning `scorecard::woebin_plot()`. 
...
We can reduce the variables that enter our feature selection process by filtering all variables with `IV < 0.02`. However we have enough computing power to use lasso on the full dataset, so we do not bother
...
car = function( formula, rsample, data, method ){
...
tib = tib %>%
  mutate( fit = pmap( list(formula, rs, data, method), car ) )
