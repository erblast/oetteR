...
The first function (regular) does the cross validation manually using the `pipelearner` approach while the second one (fast) uses the cross validation implementation that comes with the packages which makes it a lot faster. 

When building a model with a specific lambda value using the regular function , the coefficients will vary for each cross validation pair. When I built my coefficient plot I have to calculate the mean of all coefficients for a specific lambda value. I am not sure how this is handled in the first version
...
The fast version returns a model that is supposed to be built on the entire dataset for each tested lambda value. I am not sure whether is uses the fixed coefficients or if it only takes the coefficients greater than zero to fit them anew.

The fast version also calculates the the maximum lambda value whose cv error is within range of 1 SE of the lambda value with the lowest cv error.
...
lasso_fast = f_train_lasso_faster(data, form
                      , lambda = 10^seq(-3,3,length.out = 25)
                      , p = c(1,1.25,1.5,1.75,2)
                      , k = 10 )

lasso_reg = f_train_lasso(data, form
...
## Regular rtmse plot
```{r}
plotly::ggplotly(lasso_reg$plot_rtmse)
```

## Fast rtmse plot
```{r}
plotly::ggplotly(lasso_fast$plot_rtmse)
```
...
Both functions seem to give similar results

## Regular coefficients plot

```{r}
plotly::ggplotly(lasso_reg$plot_coef)
```


## Fast coefficients plot

```{r}
plotly::ggplotly(lasso_fast$plot_coef)
```
